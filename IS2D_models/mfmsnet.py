...
class MFMSNet(nn.Module):
    ...
    def structure_loss(self, pred, mask, multi_class=True):
        """
        Loss function for segmentation tasks.
        - Binary segmentation: pred (logits), mask (same size as pred)
        - Multi-class segmentation: pred (logits, [batch_size, num_classes, h, w]),
          mask (indices, [batch_size, h, w])

        Arguments:
        pred : torch.Tensor
            Predicted logits (raw scores) from the model.
        mask : torch.Tensor
            Ground truth segmentation map. For binary, same shape as pred.
            For multi-class, should be class indices of shape [batch_size, h, w].
        multi_class : bool
            If True, use multi-class segmentation logic. Otherwise, assume binary.

        Returns:
        torch.Tensor
            Scalar loss value.
        """
        # print(f"Shape of mask: {mask.shape}")  # Print the shape to diagnose

        # Handle binary and multi-class segmentation separately
        if multi_class:
            # Multi-class segmentation loss

            # If mask is 3D (batch_size, height, width)
            if len(mask.shape) == 3:
                batch_size, height, width = mask.shape  # [batch_size, height, width]
            # If mask is 4D (batch_size, channels, height, width) for multi-class segmentation
            elif len(mask.shape) == 4:
                batch_size, channels, height, width = mask.shape  # [batch_size, num_channels, height, width]
                mask = mask.squeeze(1)  # Remove channel dimension for class indices

            # Ensure mask is a LongTensor (for F.one_hot)
            mask = mask.long()

            # Apply one-hot encoding and permute dimensions to [batch_size, num_classes, height, width]
            mask_one_hot = F.one_hot(mask, num_classes=4).permute(0, 3, 1, 2).float()  # Convert to float

            # Compute weights based on ground truth
            weight_map = 1 + 5 * torch.abs(
                F.avg_pool2d(mask_one_hot, kernel_size=31, stride=1, padding=15) - mask_one_hot
            )

            # Cross-entropy loss with weighted mask
            pred_softmax = F.log_softmax(pred, dim=1)  # Log-softmax for stability
            cross_entropy_loss = -torch.sum(weight_map * pred_softmax, dim=1)
            weighted_loss = cross_entropy_loss.mean(dim=(1, 2)).mean()
            return weighted_loss
        else:
            # Binary Segmentation (default behavior)
            # If the mask has a single channel, remove the extra channel dimension
            if len(mask.shape) == 4:  # [batch_size, 1, height, width] for binary
                mask = mask.squeeze(1)  # Remove the single channel dimension

            # Ensure mask has shape [batch_size, height, width]
