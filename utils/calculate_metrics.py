...
class Fundus_Image_Segmentation_Metrics_Calculator(object):
    ...
    def calculate_DSC(self, y_pred, y_true):
        # For multi-class segmentation, apply argmax
        y_pred = torch.argmax(y_pred, dim=1)  # Get predicted class labels for each pixel
        y_true = torch.argmax(y_true, dim=1)  # If y_true is one-hot encoded, convert to class labels
        ...
        intersection = np.sum(y_true * y_pred)
        segmentation_pixels = np.sum(y_pred)
        gt_label_pixels = np.sum(y_true)
...
def Accuracy_Calculator(cfm_per_class, num_classes=4, eps=0.0001):
    accuracy_per_class = []
...
def Precision_Calculator(cfm_per_class, num_classes=4, eps=0.0001):
    precision_per_class = []
...
def Recall_Calculator(cfm_per_class, num_classes=4, eps=0.0001):
    recall_per_class = []
